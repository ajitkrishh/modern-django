...
from .forms import (
    ...
    CompanyUpdateForm,
    TransporterUpdateForm,
    DriverUpdateForm,
    VehicleUpdateForm,
    BankDetailUpdateForm,
)

from UserAccount.utils import (
    get_usertypes,
    sanitize_integer_values,
    values_list_to_values,
)
from .models import *
...
class CustomLoginView(LoginView):
    ...
    def get_success_url(self):
        ...
        usertype_action = {
            USERTYPE[0]: "company-action",
            USERTYPE[1]: "transporter-action",
            USERTYPE[2]: "owner-action",
            USERTYPE[3]: "driver-action",
        }
        if usertype in USERTYPE:
            return reverse(usertype_action[usertype])
        else:
            messages.error(self.request, "Action Not Allowed !!! ")
            return reverse("index")


@login_required
def index(req):  # index.html
    user = req.user
    utype = user.UserType
    # o1,o2,o3,o4 = [],[],[],[]
...
"""
    ...
    if utype == USERTYPE[0]:
        ...
        return render(req, "organisation/index.html", context)
        ...
        return render(req, 'transporter/index.html', context)
        ...
        print(context)
        return render(req, "owner/index.html", context)

    elif utype == USERTYPE[3]:
        user_info = user.driver
        additional_info = None
        return render(req, "driver/index.html", {'data': {"userdata": user, 'user_info': user_info, "additional_info": additional_info, "UserType": utype}})
"""
...
def update_profile(req, pk=None):
    ...
    if req.method == "POST":
        ...
        if form_type == "general":
            ...
            if img_base64:
                ...
                image_data = base64.b64decode(
                    img_base64[img_base64.find("base64") + 7 :]
                )
                ...
                form_obj = BankDetailUpdateForm(
                    req.POST, mutable_file, instance=req.user.account
                )
        ...
        return render(
            req, "userprofile/userform.html", {"bank": bank_detail, "form": form_obj}
        )


@login_required
def add_or_remove_vehicle(req):
    if req.method == "POST":
        ...
        if request_type == "1":
            ...
            if not driver_user:
                messages.error(
                    req, "No such driver exists, please register the driver !!!"
                )
            ...
            vehicle = Vehicle.objects.filter(id=vehicle_id, owner_id=req.user.id)
            if vehicle:
                vehicle = vehicle[0]
                vehicle.delete()
                messages.success("Vehicle removed successfully !!!", msg)
            return render(req, "vehicle/see_vehicles.html")

    elif req.method == "GET":
        vehicles = (
            Vehicle.objects.filter(owner=req.user)
            .select_related("driver__user")
            .values_list(
                "pk", "driver__user__first_name", "vehicle_number", "is_vehicle_active"
            )
        )
        # print(vehicles)
        vehicles = values_list_to_values(
            vehicles, ["id", "driver_name", "vh_no", "is_active"]
        )
        return render(req, "vehicle/vehicle_list.html", {"vehicles": vehicles})
...
def get_vehicle(req):
    ...
    if usertype == USERTYPE[2]:
        ...
        vehicle_info = values_list_to_values(
            vehicle_info, ["obj_id", "vehicleNumber", "stakeholder", "driver", "status"]
        )
        ...
        vehicle_info = (
            VehicleRequest.objects.filter(
                Transporter_id=transporter.id, request_status="A"
            )
            .values_list(
                "pk",
                "Vehicle__Vehicle_Number",
                "Vehicle_Owner__user__first_name",
                "Vehicle__driver__user__first_name",
                "VehicleStatus",
            )
            .order_by("Vehicle")
        )
        ...
        vehicle_info = (
            Vehicle.objects.filter(driver__user_id=req.user.id)
            .select_related("owner")
            .values_list("pk", "owner__user__first_name", "is_active", "vehicle_number")
        )
        if not vehicle_info:
            return redirect("index")

        vehicle_info = values_list_to_values(
            vehicle_info, ["obj_id", "stakeholder", "status", "vehicleNumber"]
        )
        context = {"vehicle_list": vehicle_info}
        print(context)
        return render(req, "vehicle/see_vehicles.html", context)
    ...
    if req.user.UserType == USERTYPE[1]:  # is transporter
        if req.method == "POST":
            ...
            vehicle_activate_request_obj = get_object_or_404(
                VehicleRequest,
                pk=vehicle_activate_request_id,
                Transporter=user.transporter,
            )
            if vehicle_activate_request_obj:
                if (
                    operation_type == "Activate"
                    and not vehicle_activate_request_obj.VehicleStatus
                ):
                    ...
                    and vehicle_activate_request_obj.VehicleStatus
                ):
...
def build_network(req, id=None):
    ...
    if req.method == "POST":  # when user search a name
        ...
        try:
            search_result_user = search_result_user.filter(
                UserType__in=allowed_friendship[usertype]
            )
        except:
            search_result_user = []
        ...
        if search_result_user.exists():
            ...
            search_result_user = search_result_user.filter(~Q(pk=user.pk)).values(
                "pk", "is_friend", "UserType", "get_full_name"
            )
        # print(search_result_user)
        ...
        if (
            id
        ):  # a get request fires when a connect request is made by user for another user , id refers to the to_user
            ...
            if is_allowed:
                new_friend = Friendship.objects.create(
                    from_user=user, to_user=to_User, status="P"
                )
...
def connection_requests(req):
    ...
    if req.method == "GET":
        connection_request = (
            Friendship.objects.filter(to_user=user.id, status="P")
            .select_related("from_user")
            .values_list(
                "pk", "from_user__id", "from_user__username", "from_user__UserType"
            )
        )
        connection_request = values_list_to_values(
            source=connection_request, key_list=["pk", "userid", "username", "usertype"]
        )
        ...
        friendship_obj: Friendship = get_object_or_404(
            Friendship, pk=int(friendship_obj_id)
        )
        ...
        if user_decision_type not in {"1", "2"}:
            ...
            return redirect("index")
        if (
            friendship_obj.from_user_id != int(from_user_id)
            or friendship_obj.to_user_id != req.user.id
        ):
            messages.error(req, "Operation not valid !!!")
            return redirect("index")
            ...
            if from_user_usertype in get_allowed_friendship()[usertype]:
                if usertype == USERTYPE[0]:
                    if from_user_usertype == USERTYPE[0]:
                        user.organisation.linked_Organisation.add(
                            from_user.organisation
                        )
...
def vehicle_connection_request(req):
    ...
    t_user: Transporter = user.transporter
    if req.method == "GET":
        vh_connection_request = (
            VehicleRequest.objects.filter(Transporter=t_user, request_status="P")
            .select_related("vehicle_owner", "Vehicle")
            .values_list(
                "pk",
                "Vehicle_Owner__id",
                "Vehicle_Owner__user__username",
                "Vehicle__id",
                "Vehicle__Vehicle_Number",
            )
        )
        vh_connection_request = values_list_to_values(
            source=vh_connection_request,
            key_list=["pk", "ownerid", "username", "vehicle_id", "vehicle_number"],
        )
        ...
        vehicle_req_obj: VehicleRequest = get_object_or_404(
            VehicleRequest, pk=vehicle_req_obj_id
        )
        vehicle: Vehicle = get_object_or_404(Vehicle, pk=request_vh_id)
        # print(vehicle_req_obj.__dict__ )
        if (
            vehicle_req_obj.Vehicle_Owner_id != vh_owner_id
            or vehicle.owner_id != vh_owner_id
        ):
            # print("11111")
            return redirect("index")
        if user_decision_type == 1:
