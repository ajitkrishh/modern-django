...
from .models import *
from Builti.models import Builti
...
def show(req):
    return render(req, 'common/profile_update.html')
...
def update_profile(req, pk):
    ...
    if req.method == "POST":
        ...
        if form_type == 'general':
            ...
            if (request_UserType == "Organisation"):
                form_obj = form_obj(
                    req.POST, instance=req.user.organisation)

            elif (request_UserType == "Transporter"):
                form_obj = form_obj(
                    req.POST, instance=req.user.transporter)

            elif (request_UserType == "Driver"):
                form_obj = form_obj(req.POST, instance=req.user.driver)
        ...
        bank_detail, created = BankDetail.objects.get_or_create(user=req.user)

        return render(req, "common/userprofile.html", 
                      {"bank": bank_detail, }
                      )
